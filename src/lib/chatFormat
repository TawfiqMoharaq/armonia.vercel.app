...
/** يحذف أي روابط يوتيوب من النص (روابط مباشرة أو Markdown). */
export const stripAllYoutubeLinks = (text: string): string => {
  if (!text) return text;
  // احذف روابط Markdown ليوتيوب: [title](https://youtube...)
  text = text.replace(
    /\[([^\]]+)\]\((https?:\/\/(?:www\.)?(?:youtube\.com|youtu\.be)\/[^\s)]+)\)/gi,
    "$1"
  );
  // احذف الروابط المباشرة ليوتيوب
  text = text.replace(/https?:\/\/(?:www\.)?(?:youtube\.com|youtu\.be)\/\S+/gi, "");
  // نظّف الفراغات
  return text.replace(/\n{3,}/g, "\n\n").trim();
};
  ...
  while (remaining.length) {
    ...
    if (earliest.type === "markdown") {
      ...
      nodes.push(
        React.createElement(
          "span",
          { key: `bold-${nodes.length}`, className: "font-semibold text-[#0A6D8B]" },
