...
export const sendChat = async (payload: ChatPayload): Promise<ChatResponse> => {
  try {
    ...
    const txt = (payload.user_message || "").toLowerCase();
    const reply = txt.includes("سلام")
      ? `وعليكم السلام! الخدمة متعذّرة مؤقتًا. ابدأ بإحماء خفيف 5–10 دقائق وتمارين حركة بدون ألم. بحث قد يفيدك: ${youtube}`
